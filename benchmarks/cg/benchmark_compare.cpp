...
public:
    ...
    void run_upto_convergence(::benchmark::State& state, std::array<double, 1>& timings) const requires (CollectionMethodValue == CollectionMethod::CONVERGENCE)
    {
        ...
        const auto [elapsed, res_nrm2, num_iters, sol] = T::run(*pool, state.range(0), typename T::solver_t::Parameters{.tolerance = tolerance, .max_iters = 20 /*static_cast<size_t>(state.range(1) * 2)*/});
