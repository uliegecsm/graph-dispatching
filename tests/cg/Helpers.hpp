...
struct FakeFEMLaplacian1D
{
    ...
    static FakeFEMLaplacian1D create(const utils::ExecutionSpacePool<Exec>& pool, const typename Exec::size_type size)
    {
...
struct NbyNSolverTest
{
    ...
    {
        auto system = NbyNSolverTestHelper<Exec>::initializer_t::create(pool, nrows);

        auto solver = get_solver(pool.get(0), std::move(system.mat), std::move(system.rhs));
