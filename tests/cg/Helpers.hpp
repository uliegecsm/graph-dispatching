...
struct FakeFEMLaplacian1D {
    ...
    static FakeFEMLaplacian1D create(const Exec& exec, const typename Exec::size_type size) {
        ...
        typename row_map_t::non_const_type row_map(
            Kokkos::view_alloc(Kokkos::WithoutInitializing, exec, "row map"), size + 1);
        typename entries_t::non_const_type entries(
            Kokkos::view_alloc(Kokkos::WithoutInitializing, exec, "entries"), nnz);
        typename matrix_t::values_type::non_const_type values(
            Kokkos::view_alloc(Kokkos::WithoutInitializing, exec, "values"), nnz);
...
struct NbyNSolverTest {
    ...
    requires std::is_aggregate_v<SolverType>
    ...
    static auto
        run(const utils::ExecutionSpacePool<Exec>& pool,
            const typename Exec::size_type nrows,
            const typename SolverType::Parameters& params,
...
KOKKOS_FORCEINLINE_FUNCTION auto relDifference(const Kokkos::complex<T>& val1, const Kokkos::complex<T>& val2) {
...
#define RUN_AND_CHECK(_pool_, _nrows_, _tol_, _expt_niters_, ...)                                                      \
    const auto [elapsed, res_nrm2, num_iters, sol] =                                                                   \
        this->run(_pool_, _nrows_, {_tol_, 2 * _expt_niters_} __VA_OPT__(, ) __VA_ARGS__);                             \
                                                                                                                       \
    ...
    for (typename std::remove_cvref_t<decltype(_pool_)>::execution_space::size_type irow = 0; irow < _nrows_;          \
         ++irow) {                                                                                                     \
        const Kokkos::complex<double> value{double(2 * irow) / _nrows_, double(2 * irow) / _nrows_};                   \
        EXPECT_LE(tests::cg::relDifference(mirror(irow), value), _tol_) << mirror(irow) << " not close to " << value;  \
    }
