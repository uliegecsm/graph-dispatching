...
void test_transition_from_one_to_another_with_value(Src&& src, Dst&& dst) {
    auto snd = ::stdexec::schedule(std::forward<Src>(src)) | ::stdexec::then([=]() -> int {
                   if (::nvexec::is_on_gpu())
                       return 1;
                   else
                       return 0;
               })
             | ::stdexec::continues_on(std::forward<Dst>(dst)) | ::stdexec::then([=](const int val) -> int {
...
void test_transition_from_one_to_another_no_value(Src&& src, Dst&& dst) {
    ...
    auto snd = ::stdexec::schedule(std::forward<Src>(src)) | ::stdexec::then([=] {
                   if (::nvexec::is_on_gpu())
                       ++(*ptr);
               })
             | ::stdexec::continues_on(std::forward<Dst>(dst)) | ::stdexec::then([=] {
    ...
    ASSERT_DEATH(
        {
            test_transition_from_one_to_another_with_value(
                stream_ctx.get_scheduler(), stream_ctx_another.get_scheduler());
        },
        ".*");
}
