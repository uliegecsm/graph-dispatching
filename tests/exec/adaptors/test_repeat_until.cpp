 ...
 *
 * Tests for @c exec::repeat_until
 * -------------------------------
 *
 * This group of tests check the behavior of @c exec::repeat_until.
 *
 * The tests can be found in @ref tests/exec/adaptors/test_repeat_until.cpp.
 */
...
/**
 * @test Check that the input sender to @c exec::repeat_until (or at least its data) will be copied once per repetition.
 *
 * The @c exec::repeat_until sender stores its input sender. Each re-connection copies the input sender.
...
TEST_F(RepeatEffectUntilTest, copies) {
    ...
    auto chain = ::exec::repeat_until(CHAIN | ::stdexec::then([&irep]() -> bool {
