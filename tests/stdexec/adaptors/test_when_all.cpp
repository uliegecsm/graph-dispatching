 ...
 *
 * The tests can be found in @ref tests/stdexec/adaptors/test_when_all.cpp.
 */
...
TEST(when_all, propagates_completion_domain_from_different_type_children) {
    ...
    auto domain =
        ::stdexec::get_completion_domain<::stdexec::set_value_t>(::stdexec::get_env(sndr), ::stdexec::env<>{});
...
TEST_F(WhenAllTest, on_which_thread_after) {
    ...
    auto when_all = ::stdexec::when_all(
        ::stdexec::schedule(this->pools.at(index_of<'A'>()).get_scheduler())
            | ::stdexec::then([&]() { thr_A = std::this_thread::get_id(); }),
        ::stdexec::schedule(this->pools.at(index_of<'B'>()).get_scheduler())
            | ::stdexec::then([&]() { thr_B = std::this_thread::get_id(); }),
        ::stdexec::schedule(this->pools.at(index_of<'C'>()).get_scheduler())
            | ::stdexec::then([&]() { thr_C = std::this_thread::get_id(); }));
