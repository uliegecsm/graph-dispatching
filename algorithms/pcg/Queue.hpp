#ifndef GRAPH_DISPATCHING_ALGORITHMS_PCG_QUEUE_HPP
#define GRAPH_DISPATCHING_ALGORITHMS_PCG_QUEUE_HPP
...
/**
 * @brief Preconditioned conjugate gradient solver with @c Kokkos execution space instances.
 *
...
template <
    ...
    typename AxpbyType
>
struct PCGQueue : public algorithms::cg::CGBase<MatrixType, VectorType>
{
public:
...
public:
    ...
    PCGQueue(const Exec& exec, MatType&& mat_, RhsType&& rhs_)
    ...
    {
        PLOG_INFO << "PCGQueue(apply): starting...";
        ...
        const Kokkos::Profiling::ScopedRegion loop("PCGQueue - loop");
        ...
        {
#if defined(GRAPH_DISPATCHING_ALGORITHMS_PCG_PCGQUEUE_ENABLE_SCOPEDREGION_IN_LOOP)
            const Kokkos::Profiling::ScopedRegion region("PCGQueue - iter " + std::to_string(iter));
#endif
            ...
            PLOG_INFO << "PCGQueue(apply): iteration " << iter << ", res nrm2 " << res_nrm2;
...
#endif // GRAPH_DISPATCHING_ALGORITHMS_PCG_QUEUE_HPP
