#ifndef GRAPH_DISPATCHING_ALGORITHMS_CG_QUEUE_HPP
#define GRAPH_DISPATCHING_ALGORITHMS_CG_QUEUE_HPP
...
/**
 * @brief Conjugate gradient solver with @c Kokkos execution space instances.
 *
...
template <
    ...
    typename AxpbyType
>
struct CGQueue : public CGBase<MatrixType, VectorType>
{
    ...
    std::tuple<mag_t, SizeType> apply(const utils::ExecutionSpacePool<Exec>& pool, const VectorType& sol, const typename base_t::Parameters& params) const
    {
        ...
        const Kokkos::Profiling::ScopedRegion loop("CGQueue - loop");
        ...
        {
#if defined(GRAPH_DISPATCHING_ALGORITHMS_CG_CGQUEUE_ENABLE_SCOPEDREGION_IN_LOOP)
            const Kokkos::Profiling::ScopedRegion region("CGQueue - iter " + std::to_string(iter));
#endif
...
#endif // GRAPH_DISPATCHING_ALGORITHMS_CG_QUEUE_HPP
